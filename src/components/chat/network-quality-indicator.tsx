...
const qualityMeta: Record<NetworkQuality, { bars: number; color: string; label: string }> = {
  excellent: { bars: 4, color: 'bg-success', label: 'Excellente' },
  good: { bars: 3, color: 'bg-success', label: 'Bonne' },
  fair: { bars: 2, color: 'bg-amber-500', label: 'Moyenne' },
  poor: { bars: 1, color: 'bg-destructive', label: 'Faible' },
  unknown: { bars: 0, color: 'bg-muted-foreground/40', label: 'Inconnue' },
};
    ...
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <div
            aria-label={`Qualité réseau : ${meta.label}`}
            className={cn(
        ...
        <TooltipContent side="top" className="text-[11px]">
          <div className="flex flex-col gap-0.5">
            ...
            <div className="mt-1 grid grid-cols-[auto_auto] gap-x-2 gap-y-0.5 font-mono text-[10px] text-muted-foreground">
              <span>Ping</span><span className="text-foreground">{rttLabel}</span>
              <span>Pertes</span><span className="text-foreground">{lossLabel}</span>
              <span>Gigue</span><span className="text-foreground">{jitterLabel}</span>
            </div>
          </div>
        </TooltipContent>
      </Tooltip>
