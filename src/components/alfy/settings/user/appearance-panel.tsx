...
const PRESETS: PresetMeta[] = [
  { id: 'light', nom: 'Clair', detail: 'Papier froid', mode: 'light', fond: 'oklch(97% 0.006 289)', panneau: 'oklch(100% 0.003 289)', trait: 'oklch(88% 0.008 289)' },
  { id: 'dark', nom: 'Sombre', detail: 'Le défaut', mode: 'dark', fond: 'oklch(21% 0.012 289)', panneau: 'oklch(26% 0.011 289)', trait: 'oklch(38% 0.010 289)' },
  { id: 'deep', nom: 'Sombre profond', detail: 'Contraste accru', mode: 'dark', fond: 'oklch(12% 0.008 289)', panneau: 'oklch(17% 0.010 289)', trait: 'oklch(30% 0.010 289)' },
  { id: 'oled', nom: 'Nocturne', detail: 'Noir absolu, OLED', mode: 'dark', fond: 'oklch(0% 0 0)', panneau: 'oklch(10% 0.008 289)', trait: 'oklch(24% 0.010 289)' },
  { id: 'system', nom: 'Système', detail: 'Suit l’appareil', mode: 'system', fond: 'linear-gradient(115deg, oklch(97% 0.006 289) 0 50%, oklch(12% 0.008 289) 50% 100%)', panneau: 'transparent', trait: 'transparent' },
];

const SERVER_THEME_OPTIONS = [
  { id: 'server', label: 'Utiliser le thème du serveur' },
  { id: 'user', label: 'Toujours utiliser mon thème' },
];
  ...
  return (
    <div>
      ...
      <header className="at-fade-up mb-10 flex items-start justify-between gap-6">
        <div className="min-w-0">
          <h2 className="font-heading text-3xl leading-tight tracking-tighter">Thème</h2>
          <p className="mt-2 max-w-md text-sm leading-relaxed text-muted">
            Profondeur du fond, couleurs de l&apos;interface et icône de l&apos;application.
          </p>
        </div>
      ...
      <Bloc
        titre="Profondeur"
        intro="Le socle clair ou sombre de l’interface. Il se combine librement avec la couleur choisie plus bas."
        delai={40}
      >
        {monte && (
          <ToggleButtonGroup
            isDetached
            aria-label="Profondeur du thème"
              ...
              return (
                ...
                >
                  ...
                  <span className="mt-2 block px-0.5">
                    ...
                    <span className="block text-[10px] text-muted/70">{p.detail}</span>
                  </span>
                </ToggleButton>
              );
            })}
      ...
      <Bloc
        titre="Couleur"
        intro="La teinte se propage à toute l’interface. Choisissez une ambiance, ou composez votre propre accent."
        delai={80}
        action={
          accentLibre ? (
      ...
      >
        ...
        <div className="mt-4 flex items-center justify-between gap-6 rounded-2xl bg-surface px-5 py-4">
          <div className="min-w-0">
            <p className="text-sm font-medium">Accent personnalisé</p>
            <p className="mt-0.5 text-xs leading-relaxed text-muted">
              Une couleur libre pour les boutons, les liens et le focus.
            </p>
          </div>
          <ColorPicker
          ...
          >
            ...
            <ColorPicker.Popover>
              <ColorArea aria-label="Teinte et intensité" className="max-w-full" colorSpace="hsb" xChannel="saturation" yChannel="brightness">
                <ColorArea.Thumb />
              </ColorArea>
              <ColorSlider aria-label="Nuance" channel="hue" className="gap-1 px-1" colorSpace="hsb">
                <ColorSlider.Track>
                  <ColorSlider.Thumb />
                </ColorSlider.Track>
              </ColorSlider>
      ...
      {/* ── Application ───────────────────────────────────────────────── */}
      <Bloc titre="Application" intro="Où et comment votre thème s’applique." delai={120}>
        <div className="flex flex-col gap-2">
          <Rangee
            titre="Synchroniser avec mes autres appareils"
            detail="Le thème, la profondeur et l’icône suivent votre compte."
          >
            <Bascule
              label="Synchroniser le thème avec mes autres appareils"
          ...
          <Rangee
            titre="Appliquer sur les profils des autres membres"
            detail="Ignore les couleurs de profil personnalisées."
          >
            <Bascule
              label="Appliquer le thème sur les profils des autres utilisateurs"
              isSelected={prefs.applyThemeToProfiles}
              onChange={(v) => updatePrefs({ applyThemeToProfiles: v })}
            />
          </Rangee>

          <Rangee titre="Dans les serveurs" detail="Quand un serveur impose son propre thème.">
            <Select
              className="w-64"
              aria-label="Thème par défaut dans les serveurs"
      ...
      {/* ── Icône ─────────────────────────────────────────────────────── */}
      <Bloc
        titre="Icône de l’appli"
        intro="Visible dans l’onglet du navigateur et sur l’écran d’accueil."
        delai={160}
        action={
      ...
      <section className="at-fade-up" style={{ animationDelay: '200ms' } as CSSProperties}>
        ...
        >
          ...
          <span className="min-w-0 flex-1 text-left">
            <span className="block text-sm font-medium">Accessibilité</span>
            <span className="block truncate text-xs font-normal text-muted">
              Saturation, contraste élevé, mouvement réduit, taille du texte.
            </span>
          </span>
