...
const STATUS_CFG: Record<string, { label: string; color: string; bg: string; Icon: React.ElementType }> = {
  open:        { label: 'Ouvert',     color: '#3b82f6', bg: '#3b82f618', Icon: InboxIcon },
  pending:     { label: 'En attente', color: '#f59e0b', bg: '#f59e0b18', Icon: ClockIcon },
  in_progress: { label: 'En cours',   color: '#8b5cf6', bg: '#8b5cf618', Icon: ZapIcon },
  resolved:    { label: 'Résolu',     color: '#22c55e', bg: '#22c55e18', Icon: CheckCircle2Icon },
  closed:      { label: 'Fermé',      color: '#6b7280', bg: '#6b728018', Icon: XCircleIcon },
};
...
function StatusBadge({ status }: { status: string }) {
  const cfg = STATUS_CFG[status] ?? { label: status, color: '#6b7280', bg: '#6b728018', Icon: InboxIcon };
  const Icon = cfg.Icon;
  return (
...
function Avatar({ name, isStaff }: { name: string; isStaff: boolean }) {
  if (isStaff) {
    return (
      <div className="size-8 rounded-full flex items-center justify-center shrink-0 bg-primary/15">
  ...
  const sendReply = async () => {
    ...
    try {
      ...
      if (r.success) {
        ...
        if (mr.success) setMessages((mr.data as any).data ?? mr.data ?? []);
        if (ticket.status === 'resolved') setTicket(tk => tk ? { ...tk, status: 'open' } : tk);
    ...
    <div className="flex flex-col h-screen bg-background">
          ...
          {ticket ? (
            <>
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className="text-[11px] text-muted-foreground/60 font-mono">#{ticket.ticketNumber}</span>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center gap-3">
            <AlertTriangleIcon size={28} className="text-destructive opacity-50" />
                  ...
                  <div key="original">
                    ...
                    <div className="flex gap-3 justify-end mb-1 mt-3">
                      <div className="flex flex-col items-end max-w-[75%]">
                        ...
                        <span className="text-[11px] text-muted-foreground/60 mt-1 px-1">{formatTime(item.date)}</span>
                      </div>
                    ...
                    <div className={`flex flex-col max-w-[75%] ${isMe ? 'items-end' : 'items-start'}`}>
                      {!groupWithPrev && !isMe && (
                        <div className="flex items-center gap-1.5 mb-1 px-1">
                          ...
                          <WrenchIcon size={10} className="text-primary/50" />
                        </div>
                      )}
                      ...
                      <span className="text-[11px] text-muted-foreground/60 mt-1 px-1">{formatTime(msg.createdAt)}</span>
                    </div>
                    {isMe && (
                      <div className="w-8 shrink-0">
      ...
      {ticket && ticket.status !== 'closed' && (
        <div className="shrink-0 px-4 pb-4 pt-2 border-t border-border/40">
          ...
          <div className="flex items-end gap-2 rounded-xl border border-border/60 bg-card px-4 py-3 focus-within:border-primary/40 transition-colors">
            <textarea
              ref={textareaRef}
              rows={1}
              ...
              className="flex-1 bg-transparent text-sm resize-none outline-none placeholder:text-muted-foreground/60 leading-relaxed py-0"
              style={{ height: '22px', maxHeight: '120px' }}
            />
            <button
              onClick={sendReply}
              disabled={!reply.trim() || sending}
              className="size-8 rounded-lg bg-primary flex items-center justify-center text-primary-foreground shrink-0 hover:bg-primary/90 disabled:opacity-40 disabled:cursor-not-allowed active:scale-95 transition-all"
            >
              {sending
      ...
      {ticket && ticket.status === 'closed' && (
        <div className="shrink-0 px-4 pb-4 pt-2 border-t border-border/40">
          <div className="rounded-xl border border-border/40 bg-muted/20 px-4 py-3 text-center text-xs text-muted-foreground">
            Ce ticket est fermé.{' '}
            <Link href="/support/contact" className="text-primary hover:underline underline-offset-2">
...
function DateSeparator({ date }: { date: string }) {
  return (
    <div className="flex items-center gap-3 my-5">
      ...
      <span className="text-[11px] text-muted-foreground/60 shrink-0">{formatDateSep(date)}</span>
      <div className="flex-1 h-px bg-border/50" />
    </div>
  );
}
